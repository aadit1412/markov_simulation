...
team_matrices = {}
for team in group_stage_teams:
    counts = build_count_matrix(df, team, team_group_matches[team])
...
calibrate(team_matrices['Argentina'], team_matrices['France'])

calibrate(team_matrices['Morocco'], team_matrices['Croatia'])
...
def get_winner(home_team, away_team, sim):
    if sim['home_win_pct'] > sim['away_win_pct']:
        ...
        return home_team if sim['avg_home_goals'] > sim['avg_away_goals'] else away_team
...
CORRECT = {
    ('Netherlands',  'United States'): False,
...
# ── threshold: only draw arrows above this probability to avoid clutter
    ...
    for from_state in TRANSIENT:
        for to_state in TRANSIENT:
            ...
            if from_state == to_state:
                ...
                # offset arrows slightly so A→B and B→A don't overlap
                dx, dy  = x2 - x1, y2 - y1
